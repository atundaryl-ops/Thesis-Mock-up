...
import { 
  ...
  Bell, Upload, CheckCircle, Loader2, AlertCircle, CreditCard
} from 'lucide-react';
...
const VehicleOwnerPortal = ({ onLogout }) => {
  ...
  const [violationFilter, setViolationFilter] = useState('unpaid');
  ...
  // Overdue: unpaid violations older than 30 days
  const isOverdue = (v) => {
    if (v.status !== 'unpaid') return false;
    const violDate = new Date(v.date);
    const now = new Date();
    return (now - violDate) / (1000 * 60 * 60 * 24) > 30;
  };

  const filteredViolations = myViolations.filter(v => {
    if (violationFilter === 'all') return true;
    if (violationFilter === 'unpaid') return v.status === 'unpaid';
    if (violationFilter === 'paid') return v.status === 'paid';
    if (violationFilter === 'dispute') return v.status === 'disputed';
    return true;
  });
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-scale-in">
        ...
        <div className="space-y-3 mb-4">
          ...
          <button className="w-full p-4 border rounded-xl flex items-center gap-3 hover:bg-slate-50 transition">
            <span className="w-6 h-6 text-slate-600 font-bold text-lg flex items-center justify-center">₱</span>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-4 space-y-3">
        {loading ? (<><Skeleton className="h-28 rounded-xl" /><Skeleton className="h-20 rounded-xl" /></>) : (
          <>
            {/* Priority: Unpaid alert banner */}
            {stats.unpaid > 0 && (
              <div className="bg-gradient-to-r from-rose-500 to-orange-500 rounded-xl p-4 text-white shadow-lg shadow-rose-500/30">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-white/90 text-sm">You have {stats.unpaid} unpaid violation{stats.unpaid !== 1 ? 's' : ''}</p>
                  </div>
            ...
            {stats.unpaid === 0 && (
              <div className="bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-xl p-4 text-white shadow-lg shadow-emerald-500/20">
                <div className="flex items-center gap-3">
                  <CheckCircle className="w-8 h-8" />
                  <div>
                    <p className="font-bold">All Clear!</p>
                    <p className="text-white/80 text-sm">No unpaid violations. Keep it up!</p>
                  </div>
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white rounded-xl p-4 shadow-sm border">
                ...
                <p className="text-xs text-slate-500">Unpaid Violations</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm border">
                <span className="w-5 h-5 text-amber-500 mb-2 font-bold text-base flex items-center">₱</span>
      ...
      {activeTab === 'violations' && (
        <div className="px-4 mb-3 flex gap-2">
          {[
            ...
            { id: 'dispute', label: 'Disputed', color: 'amber' },
            { id: 'all', label: 'All', color: 'blue' },
          ].map(f => (
            <button key={f.id} onClick={() => setViolationFilter(f.id)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition border ${
                violationFilter === f.id
                  ...
                  : f.id === 'dispute' ? 'bg-amber-500 text-white border-amber-500'
                  : 'bg-blue-600 text-white border-blue-600'
                  : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
              }`}>
              {f.label}
              <span className="ml-1 opacity-80">
                ({f.id === 'all' ? myViolations.length
                  : f.id === 'unpaid' ? myViolations.filter(v => v.status === 'unpaid').length
                  : f.id === 'paid' ? myViolations.filter(v => v.status === 'paid').length
                  : myViolations.filter(v => v.status === 'disputed').length})
              </span>
            </button>
          ))}
        </div>
            ...
            {activeTab === 'violations' && (
              filteredViolations.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-xl border shadow-sm">
                  ...
                  <p className="text-slate-500 font-medium">No {violationFilter !== 'all' ? violationFilter : ''} violations</p>
                </div>
              ) : filteredViolations.map((v) => {
                const overdue = isOverdue(v);
                return (
                  <div key={v.id} className={`bg-white rounded-xl p-4 shadow-sm border ${overdue ? 'border-rose-400 ring-2 ring-rose-200' : ''}`}>
                    {overdue && (
                      <div className="flex items-center gap-2 mb-3 bg-rose-50 border border-rose-200 rounded-lg px-3 py-2">
                        <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
                        <p className="text-xs font-semibold text-rose-700">⚠ OVERDUE — Please settle immediately to avoid penalties</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between pt-3 border-t">
                      <p className={`text-lg font-bold ${overdue ? 'text-rose-600' : ''}`}>₱{v.fine.toLocaleString()}</p>
                      <div className="flex gap-2">
                        ...
                        {v.status === 'unpaid' && (
                          <>
                            ...
                            <button onClick={() => { setSelectedViolation(v); setShowPayment(true); }} className={`px-4 py-2 text-sm text-white rounded-lg ${overdue ? 'bg-rose-500 hover:bg-rose-600 animate-pulse' : 'bg-emerald-500 hover:bg-emerald-600'}`}>Pay Now</button>
                          </>
                        )}
                      </div>
                    </div>
